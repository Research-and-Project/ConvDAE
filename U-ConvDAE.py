...
# mid
    ...
    mid2 = tf.layers.conv2d(mid1, 64, (3,3), padding='same', activation=act_fun) # pic_size/8*64
    
# decoder
with tf.name_scope('decoder'):
    deconv_weight_1 = tf.Variable(tf.truncated_normal(shape=[3, 3, 64, 64], stddev=0.1), name='deconv_weight_1') 
    deconv1 = deconv(mid2, deconv_weight_1, [batch_size, np.ceil(pic_size[0]/4), np.ceil(pic_size[1]/4), 64], [1, 2, 2, 1]) # pic_size/4*64
    concat1 = tf.concat([deconv1, conv3], 3) # pic_size/4*128
    deconv_1 = tf.layers.conv2d(concat1, 64, (3,3), padding='same', activation=act_fun) #pic_size/4*64
    ...
    deconv2 = deconv(deconv_1, deconv_weight_2, [batch_size, np.ceil(pic_size[0]/2), np.ceil(pic_size[1]/2), 64], [1, 2, 2, 1]) # pic_size/2*64
    concat2 = tf.concat([deconv2, conv2], 3) # pic_size/2*128
    deconv_2 = tf.layers.conv2d(concat2, 32, (3,3), padding='same', activation=act_fun) # pic_size/2*32
