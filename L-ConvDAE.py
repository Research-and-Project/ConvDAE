...
# To simplify the codes, directly use keep_prob_v(or its expression) as mask layer's keep probability
...
model_dir = "L-ConvDAE--" +  timestamp
model_path = model_root_path + model_dir

if not os.path.isdir(model_path):
   os.makedirs(model_path)

train_log_dir = 'logs/train/L-ConvDAE-pic_'+timestamp
test_log_dir = 'logs/test/L-ConvDAE-pic_'+timestamp
...
if in_data_flag==2:    
    path1 = "MNIST_diff/MNIST_diff_train.mat"
    path2 = "MNIST_diff/MNIST_diff_test.mat"
...
with tf.name_scope('encoder'):
    if unsup==1:
        inputs_ = tf.nn.dropout(inputs_, keep_prob-0.3) #无监督：去噪自编码随机mask输入层
